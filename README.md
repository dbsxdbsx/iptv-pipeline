...
- **真实媒体深验**：FFprobe 识别视频轨/codec/分辨率，FFmpeg 下载子资源并解码数秒；无自定义请求头的线路还必须通过 GStreamer discoverer，进一步贴近小董电视播放栈。
...
`stable` 的含义是“从该轮 GitHub 托管 runner 网络视角可由 FFmpeg 解码，并在无需自定义请求头时通过 GStreamer discoverer”，不是对所有国家、运营商或终端平台的绝对可播承诺。当前 runner 无法验证的 IPv6 / 非 HTTP 流只保留在 `all.m3u`。
...
- [x] FFprobe 元数据检查 + FFmpeg 短时解码 + GStreamer discoverer，freezedetect 作为软质量信号
...
- **FFmpeg 与 GStreamer**：无自定义头线路同时经过两套栈；需 UA/Referer 的线路因 `gst-discoverer` CLI 无法注入 HLS 子请求头，仅执行 FFmpeg 门禁并依赖 App 真实首帧学习。Windows/Android 与 Linux runner 仍可能存在 TLS/CDN/插件差异。
